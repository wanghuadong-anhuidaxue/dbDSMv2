...
from App.tools import random_filename, os, Path
...
def findMutation(filename, mutationPath, resultfilePath):
    ...
    with open(mutationPath, 'r') as f1, \
        ...
        for muta in f1:
            muta = muta.strip('\n').split('\t')
            # print(muta)
            cmdStr = 'tabix /data1/WWW/flask_website/dbDSMv2/App/static/download/analysisData/whole_genome_GRCh37_sSNV_featanno.vcf.gz ' + str(
...
@analysisblue.route('/dbDSMv2/analysis')
def analysisIndex():
    form = AnalysisForm()
...
@analysisblue.route('/dbDSMv2/analysisSubmit', methods=['GET', 'POST'])
def analysisSubmit():
    form = AnalysisForm()
    ...
    if form.email.data:
        ...
        web = 'http://bioinfo.ahu.edu.cn:5009/downResult?filename='+filename
